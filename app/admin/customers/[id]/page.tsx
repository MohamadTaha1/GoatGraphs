...
import { ArrowLeft, Loader2, Mail, Phone, MapPin, Calendar, ShoppingBag, User, Check, X, RefreshCw } from "lucide-react"
...
export default function CustomerDetailPage({ params }: { params: { id: string } }) {
  ...
  const handleRefresh = () => {
    ...
    toast({
      ...
      description: "The customer data is being refreshed.",
    })
  }
            ...
            <div className="flex flex-col items-center text-center">
              <Avatar className="h-24 w-24 mb-4 bg-black border-2 border-gold">
                {customer.photoURL ? (
                  ...
                  <AvatarFallback className="text-gold font-display text-2xl">
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="font-body text-offwhite">{customer.id}</p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="font-body text-offwhite">{customer.email}</p>
                </div>
              </div>
              ...
              {customer.phoneNumber && (
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="font-body text-offwhite">{customer.phoneNumber}</p>
                  </div>
                </div>
              )}
              ...
              {customer.address && (
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="font-body text-offwhite">{customer.address.line1}</p>
                    {customer.address.line2 && <p className="font-body text-offwhite">{customer.address.line2}</p>}
                    <p className="font-body text-offwhite">
                      {customer.address.city}, {customer.address.state || ""} {customer.address.postalCode}
                    </p>
                    <p className="font-body text-offwhite">{customer.address.country}</p>
                  </div>
                </div>
              )}
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="font-body text-offwhite">{formatDate(customer.lastLogin)}</p>
                </div>
              </div>
            </div>
          ...
          <CardContent>
            {customerOrders.length > 0 ? (
                  ...
                  <TableBody>
                    {customerOrders.map((order) => (
                      <TableRow key={order.id} className="border-gold/30">
                        <TableCell className="font-display text-offwhite">{order.id}</TableCell>
                        <TableCell className="font-body text-offwhite">{formatDate(order.createdAt)}</TableCell>
                        <TableCell className="font-body text-offwhite">{order.items?.length || 0}</TableCell>
                        <TableCell className="font-body text-offwhite">${formatPrice(order.total || 0)}</TableCell>
                        <TableCell>
              ...
              <div className="text-center py-12">
                ...
                <h3 className="text-lg font-display font-bold mb-2 text-offwhite">No Orders Yet</h3>
                <p className="text-offwhite/70 font-body mb-4">This customer hasn't placed any orders yet.</p>
                  ...
                  {Object.entries(customer).map(([key, value]) => {
                    ...
                    return (
                      <TableRow key={key} className="border-gold/30">
                        <TableCell className="font-semibold text-offwhite">{key}</TableCell>
                        <TableCell className="text-offwhite">{displayValue}</TableCell>
                      </TableRow>
                    )
                  })}
                </TableBody>
                  ...
                  <Table>
                    ...
                    <TableBody>
                      {Object.entries(customer.address).map(([key, value]) => (
                        <TableRow key={key} className="border-gold/30">
                          <TableCell className="font-semibold text-offwhite">{key}</TableCell>
                          <TableCell className="text-offwhite">{value ? String(value) : "N/A"}</TableCell>
                        </TableRow>
                      ))}
                    </TableBody>
                  </Table>
                    ...
                    <TableBody>
                      {customer.wishlist.map((productId: string) => (
                        <TableRow key={productId} className="border-gold/30">
                          <TableCell className="text-offwhite">{productId}</TableCell>
                        </TableRow>
                      ))}
                    </TableBody>
                  </Table>
