  ...
  return (
    <div className="container mx-auto py-8">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
        <h1 className="text-3xl font-bold">Football Jersey Auctions</h1>
      ...
      <Tabs defaultValue="active" className="mb-8" onValueChange={setActiveTab}>
        ...
        <TabsContent value="active" className="mt-6">
          {filteredAuctions.length === 0 ? (
            ...
            <div className="space-y-4">
              {filteredAuctions.map((auction) => (
                <div key={auction.id} className="flex flex-col md:flex-row gap-4 border rounded-lg p-4">
                  <div className="md:w-1/3 lg:w-1/4">
                    <AuctionItemCard key={auction.id} item={auction} onBidPlaced={handleBidPlaced} />
                  </div>
                  <div className="md:w-2/3 lg:w-3/4">
                    <h3 className="text-xl font-bold">{auction.playerName}</h3>
                    <p className="text-gray-500">{auction.team}</p>
                    <div className="mt-4 flex justify-between">
                      <div>
                        <p className="text-sm text-gray-500">Current Bid</p>
                        <p className="text-xl font-bold text-amber-600">${auction.currentBid.toFixed(2)}</p>
                      </div>
                      <div>
        ...
        <TabsContent value="ended" className="mt-6">
          {filteredAuctions.length === 0 ? (
            <div className="text-center py-12">
              <p className="text-lg text-gray-500">No ended auctions found</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
              {filteredAuctions.map((auction) => (
